...
def main():
    ...
    ap.add_argument("--fallback-band", type=float, default=0.3,
                    help="동일단가 무매칭 시 ±비율 가격대로 근사 경쟁사(0=끔, 기본 0.3=±30%%)")
    ap.add_argument("--workers", type=int, default=8, help="동시 매칭 스레드 수")
    a = ap.parse_args()
    ...
    print(f"{len(companies)}개 업체 / {total}개 품목 → 동일단가 경쟁사 매칭 "
          f"(계열사 상호 제외, 동시 {a.workers}, 무매칭 시 ±{a.fallback_band:.0%} 근사)")
    ...
    build_report.build_multi(companies, a.out, top_n=a.top, exact_price=True,
                             ...
                             fallback_band=a.fallback_band, workers=a.workers)
    print("저장:", a.out)
