...
```
후보 풀  = 동일단가(하드 필터) + 키워드(품명), 자기 업체 제외
유사도   = 0.55·재질동일 + 0.20·용도유사 + 0.10·규격근접 + 0.15·가격
           └ 재질이 1순위(동일 재질이 항상 상위) · 규격은 ±35% 이내면 만점(비슷하면 OK)
```
재질 표기는 대분류로 정규화(적삼목→목재, 스텐/SUS→스테인리스 등). `matcher.WEIGHTS`·`SPEC_TOL`로 조정. 완전 자동 확정이 아니라 **top-N 후보를 추려 사람이 검수하는 반자동**을 전제로 한다.
